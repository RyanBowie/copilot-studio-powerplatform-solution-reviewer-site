...
<div class="section-heading"><p class="eyebrow">Measured follow-up · 14 September 2026</p><h2 id="coverage-plan-title">More accepted assessments.<br>A complete, reliable MAIN assembly.</h2><p>The latest revision improved from the historical <strong>1/5 to 3/5 accepted assessments</strong>, with valid MAIN in both unchanged-revision runs. That is 60% rather than 20%—more accepted than unavailable. It is <strong>not an app pass rate</strong>, and the original 4/5 target remains unmet.</p></div>
...
<div class="actions"><a class="button primary" href="follow-up.html">Read the complete new assessment →</a><a class="button secondary" href="{PREFIX}/complete-review.txt" download>Complete new MAIN (.txt) ↓</a><a class="button secondary" href="{PREFIX}/results.json">All measured results</a></div>
<div class="two-up">
...
<p class="notice">The screenshot journey, original four TXT reports, eight images and 14-file download above are unchanged historical evidence. The new complete redacted MAIN is separately published. No new email receipt, flow-run screenshot, installable package or cross-tenant certification is claimed.</p>
</section>'''
...
<main class="wrap"><section class="example-intro"><p class="eyebrow">Same genuine export · separately measured follow-up</p><h1>Complete source supplied.<br><span>Partial assessment retained.</span></h1>
<p class="example-lead">Revision {html.escape(latest["revision"])} accepted {latest["accepted"]}/5 selected-source assessments, with a valid assembled MAIN. Both unchanged-revision runs accepted 3/5. The 4/5 combined target remains unmet.</p>
...
<p class="fine-print">TXT/JSON remains the runtime format. Browser Print / Save as PDF is a presentation option; an automated Word/PDF formatting flow is not implemented. <a href="{PREFIX}/provenance.json">Reviewed public-file hashes</a> · <a href="{PREFIX}/results.json">Every measured comparison</a>.</p></section>
